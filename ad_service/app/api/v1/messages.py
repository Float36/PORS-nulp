...
from app.crud.crud_messages import CRUDMessage
...
async def create_message(message_in: MessageCreate, session: AsyncSession = Depends(get_db_session)):
    """Create message (validates sender_id via user_service)."""
    service = CRUDMessage(session)
...
async def update_message(
    message_id: int, message_in: MessageUpdate, session: AsyncSession = Depends(get_db_session)
):
    service = CRUDMessage(session)
